...
fn main() {
    ...
    // Value-array fetch fuzzlib (Slice 3): reuse assemble_dir3.py with the fuzzlib driver.
    let status_dir3 = Command::new("python3")
        .arg(cref.join("assemble_dir3.py"))
        .env("TIFF_SRC", &tiff_src)
        .env("OUT", &out_dir)
        .env("DRIVER", "_fuzzlib_driver_dir3.c")
        .status()
        .expect("run cref/assemble_dir3.py (fuzzlib variant)");
    assert!(status_dir3.success(), "assemble_dir3.py (fuzzlib) failed");

    // Long8 strip-array fuzzlib (Slice 4): reuse assemble_dir4.py with the fuzzlib driver.
    let status_dir4 = Command::new("python3")
        .arg(cref.join("assemble_dir4.py"))
        .env("TIFF_SRC", &tiff_src)
        .env("OUT", &out_dir)
        .env("DRIVER", "_fuzzlib_driver_dir4.c")
        .status()
    ...
    cc::Build::new()
        .compiler("clang")
        .file(Path::new(&out_dir).join("legacy_fuzzlib_dir3.c"))
        .flag("-fsanitize=address,fuzzer-no-link")
        .flag("-g")
        .flag("-O1")
        .compile("tiff_fuzzlib_dir3");
    cc::Build::new()
        .compiler("clang")
        .file(Path::new(&out_dir).join("legacy_fuzzlib_dir4.c"))
        .flag("-fsanitize=address,fuzzer-no-link")
        .flag("-g")
        .flag("-O1")
