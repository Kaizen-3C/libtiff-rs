/* Self-contained port target: libtiff 4.7.0 value-array fetch (Slice 3) — the core of
   TIFFFetchNormalTag's field read: TIFFReadDirEntryByteArray -> TIFFReadDirEntryArray ->
   TIFFReadDirEntryArrayWithLimit. This is the count x typesize MULTIPLY-OVERFLOW +
   MAX_SIZE_TAG_DATA (2GB) size-sanity + allocation-bounds + classic/BigTIFF inline-vs-offset
   read + per-element range-checked type conversion. libtiff's tag-array integer-overflow /
   under-allocation -> OOB CVE surface. Driven over a memory buffer (isMapped always true; the
   non-mapped Seek/Read + DataAndRealloc path is stubbed, never taken).

   Structured fuzz/driver input (both C and Rust interpret identically):
     [0]=flags(bit0 SWAB, bit1 BIGTIFF)  [1..3]=tdir_type(u16 native)
     [3..11]=tdir_count(u64 native)      [11..19]=tdir_offset(8 raw bytes as the union holds them)
     [19..]=file (tif_base)
